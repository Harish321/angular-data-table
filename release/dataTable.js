...
(function (global, factory) {
    ...
    function RowDirective() {
        return {
            ...
            link: function link($scope, $elm, $attrs, ctrl) {
                var temp = $scope.rowCtrl.tabindex;
                temp = Number(temp);
                temp = temp+1;
    ...
    var DataTableController = function () {
        ...
        function DataTableController($scope, $filter, $log, $transclude) {
            ...
            $scope.onAttrClicked = function (event) {
                ...
                $(event.currentTarget).parent().next('.dt-row').slideToggle();
                if( $(event.currentTarget).parent().next('.dt-row').hide()){
                     $(event.currentTarget).parent().next('.dt-row').show();
                    $(event.currentTarget).parent().next('.dt-row').css("dispaly","grid");
                }
                else{
                    $(event.currentTarget).parent().next('.dt-row').hide();
                }
                        ...
                        angular.element($window).on('resize', throttle(function () {
                           if(typeof cordova == 'undefined'){  
                            if($window.innerWidth>=900){
                                var $temp = $('.attrname');
                                $($temp).parent().next('.dt-row').css("display","grid");
                                }
                            
                            else{
                                    var $temp = $('.attrname');
                                $($temp).parent().next('.dt-row').hide();
                                }
                            }
